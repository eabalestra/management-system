...
class FindProducts
  ...
  def initial_scope
    Product.with_attached_image_url
  end
  ...
  def sort(scoped, order_by)
    order_by_query = Product::ORDER_BY.fetch(order_by, { name: :asc })
